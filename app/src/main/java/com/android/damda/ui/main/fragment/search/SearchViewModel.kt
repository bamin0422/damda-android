...
class SearchViewModel : ViewModel() {
    val searchData = MutableLiveData<MutableList<ImgItem>>()
    private var data = mutableListOf<ImgItem>()

    /**
     * getSearchData : LiveData인 memoryData의 value 값을 리턴
    ...
    /**
     * addSearchData : 특정 ImgItem 추가하는 function
     * */
    fun addSearchData(memData : ImgItem){
        data.add(memData)
        searchData.value = data
    }

    /**
     * addSearchDatas : 여러 ImgItem 추가하는 function
     * */
    fun addSearchDatas(memoryDatas : MutableList<ImgItem>){
        memoryDatas.forEach{
            data.add(it)
        }
        searchData.value = data
    ...
    /**
     * deleteSearchData : 특정 ImgItem 삭제하는 function
     * */
    fun deletePhotoData(memData : ImgItem){
        data.remove(memData)
        searchData.value = data
    }

    /**
     * deletePhotoDatas : 여러 ImgItem 삭제하는 function
     * */
    fun deleteSearchDatas(sDatas : MutableList<ImgItem>){
        sDatas.forEach{
            data.remove(it)
        }
        searchData.value = data
